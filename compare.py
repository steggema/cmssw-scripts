...
def comparisonPlots(u_names, trees, titles, pname='sync.pdf', ratio=True, weight='1'):
...
'''
    ...
    parser.add_option('-w', '--weight', dest='weight', default='1', help='Use weight (can be a selection)')
    ...
    comparisonPlots(u_names, trees, titles, 'sync.pdf', options.do_ratio, weight=options.weight)

    if 'evt' not in u_names:
        sys.exit()
    ...
    if len(trees) == 2 and options.do_intersect:
        ...
        if not all(l.GetN() == 0 for l in intersect):
            comparisonPlots(u_names, trees, titles, 'intersect.pdf', options.do_ratio, weight=options.weight)
    ...
    if len(trees) == 2 and options.do_common:
        ...
        trees[0].SetEntryList(intersect[0])
        trees[1].SetEntryList(intersect[1])
        comparisonPlots(u_names, trees, titles, 'common.pdf', options.do_ratio, weight=options.weight)
