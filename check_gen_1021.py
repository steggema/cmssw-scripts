...
events = Events('miniAOD_TauReco_ak4PFJets_tentaus.root')
    ...
    for gen_tau in vis_had_taus:
        if gen_tau.pt() > 25. and abs(gen_tau.eta()) < 2.3:
            ...
            for tau1 in taus:
                if deltaR(tau1, gen_tau) < 0.3:
                    ...
                    if tau1.pt() > 0.8 * gen_tau.pt():
                        matched_tau1 = tau1
            for tau2 in taus2:
                if deltaR(tau2, gen_tau) < 0.3:
                    best_tau2 = tau2
                    if tau2.pt() > 0.8 * gen_tau.pt():
                        matched_tau2 = tau2

            if gen_tau.dm == 1:
                print '\n ## gen tau {:.2f} {:.2f} {:.2f} {:.2f} {}'.format(gen_tau.pt(), gen_tau.eta(), gen_tau.phi(), gen_tau.mass(), gen_tau.dm)
                if best_tau1:
                    print ' ## best tau 1 {:.2f} {:.2f} {:.2f} {:.2f} {}'.format(best_tau1.pt(), best_tau1.eta(), best_tau1.phi(), best_tau1.mass(), best_tau1.decayMode())
                if best_tau2:
                    print ' ## best tau 2 {:.2f} {:.2f} {:.2f} {:.2f} {}'.format(best_tau2.pt(), best_tau2.eta(), best_tau2.phi(), best_tau2.mass(), best_tau2.decayMode())
                if best_tau1 and best_tau2 and best_tau1.pt() - gen_tau.pt() > 50.:# abs(best_tau1.pt() - best_tau2.pt()) > 0.3:
                    for ch in gen_tau.charged:
                        print ' gen ch {:.2f} {:.2f} {:.2f} {:.2f}'.format(ch.pt(), ch.eta(), ch.phi(), deltaR(ch, gen_tau))
                    for pi0 in gen_tau.pizeros:
                        print ' gen pizero'
                        for d in pi0:
                            print '  gamma {:.2f} {:.2f} {:.2f} {}'.format(d.pt(), d.eta(), d.phi(), d.pdgId())
                    print 'Reco tau 1'
                    for ph in best_tau1.signalChargedHadrCands():
                        print '  rec ch {:.2f} {:.2f} {:.2f} {}'.format(ph.pt(), ph.eta(), ph.phi(), ph.pdgId())
                    for ph in best_tau1.signalGammaCands():
                        print '  rec gamma {:.2f} {:.2f} {:.2f} {:.2f}'.format(ph.pt(), ph.eta(), ph.phi(), deltaR(ph, best_tau1.leadChargedHadrCand()))
                    print 'Reco tau 2'
                    for ph in best_tau2.signalChargedHadrCands():
                        print '  rec ch {:.2f} {:.2f} {:.2f} {}'.format(ph.pt(), ph.eta(), ph.phi(), ph.pdgId())
                    for ph in best_tau2.signalGammaCands():
                        print '  rec gamma {:.2f} {:.2f} {:.2f} {:.2f}'.format(ph.pt(), ph.eta(), ph.phi(), deltaR(ph, best_tau2.leadChargedHadrCand()))
                    print 'PF e/gamma cands'
                    for pf in pfcands:
                        if deltaR(gen_tau, pf) < 0.3 and abs(pf.pdgId()) in [11, 22]:
                            print '    {:.2f} {:.2f} {:.2f} {:.2f} {}'.format(pf.pt(), pf.eta(), pf.phi(), deltaR(pf, gen_tau.charged[0]), pf.pdgId())
                            # if pf.charge():
                            #     print '     dz, dxy {:.2f} {:.2f}'.format(pf.dz(), pf.dxy())
                    import pdb; pdb.set_trace()
            ...
            if matched_tau1:
                ...
                # FOLLOWING CODE NEED EXTRA INFO
